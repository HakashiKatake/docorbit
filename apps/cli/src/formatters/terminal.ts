...
export function formatInspectionReport(report: InspectionReport): string {
  ...
  lines.push('');
  lines.push(`${BOLD}${CYAN}DocRouter${RESET} ${DIM}— Documentation Intelligence Report${RESET}`);
...
export function formatIngestionResult(result: IngestionResult): string {
  ...
  lines.push('');
  lines.push(`${BOLD}${GREEN}DocRouter${RESET} ${DIM}— Documentation Ingested Successfully${RESET}`);
  lines.push(`${DIM}${'═'.repeat(64)}${RESET}`);
